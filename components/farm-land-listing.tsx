...
export function FarmLandListing() {
  ...
  const listing = {
    ...
    location: "Heartland County, Midwest State",
    soilType: "Deep, well-drained loam",
    topography: "Gently rolling",
    waterSource: "On-site well and creek",
    previousCrops: ["Corn", "soybeans", "wheat rotation"],
    rentalOptions: [
      {
        value: "lease",
        label: "Traditional Lease",
        averageMarketRate: `${averageMarketPrices.lease[rentalDuration]}/acre/year for ${rentalDuration} year(s)`
      },
      {
        value: "cropShare",
        label: "Crop Share",
        averageMarketRate: `${averageMarketPrices.cropShare}% crop share for landowner`
      },
      {
        value: "flexRent",
        label: "Flex Rent",
        averageMarketRate: `${averageMarketPrices.flexRent.base}/acre base + ${averageMarketPrices.flexRent.bonus}% bonus`
      }
    ]
  };

  const averageMarketPrices: { [key: string]: any } = {
            ...
            <CardContent className="space-y-4">
              ...
              <div>
                ...
                <ul className="text-sm text-gray-600 space-y-1">
                  <li>Soil Type: {listing.soilType}</li>
                  <li>Topography: {listing.topography}</li>
                  <li>Water Source: {listing.waterSource}</li>
                  <li>Previous Crops: {listing.previousCrops.map((crop) => <li key={crop}>{crop}</li>)}</li>
                </ul>
              </div>
            </CardContent>
          </Card>
